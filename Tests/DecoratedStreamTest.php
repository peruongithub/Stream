...
use Trident\Component\Stream\Decorator\DecoratedStream;
use Trident\Component\Stream\Stream;

/**
 * @covers Trident\Component\Stream\Stream
 */
class DecoratedStreamTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $stream1 = $this->getStream(new Stream(fopen(__FILE__, 'r')));
        $stream2 = $this->getStream(new Stream(fopen(__FILE__, 'r')));
        $stream1->writeStream($stream2);
    }

    public function testWriteStream()
    {
        $stream1 = $this->getStream(new Stream(fopen('php://temp', 'r+')));
        $stream2 = $this->getStream(new Stream(fopen(__FILE__, 'r')));
        $stream1->writeStream($stream2);
    ...
    public function testEnsuresSizeIsConsistent()
    {
        ...
        $stream = $this->getStream(new Stream($h));
    ...
    {
        $r = fopen('php://temp', 'w+');
        $stream = $this->getStream(new Stream($r));
        $stream->write('foo');
        $this->assertTrue($stream->isReadable());
    ...
    {
        $stream = $this->getStream(new Stream(fopen(__FILE__, 'r')));
        $stream->write('bar');
    }
